...
def create_editorial_features(df):
    ...
    print("  Creating headline quality indicators...")
    ...
    print("  Computing advanced headline metrics...")
    ...
    print("  Creating editorial scoring metrics...")
    # Editorial Scores
    ...
    if "ctr" in df.columns and not df["ctr"].isna().all():
        ...
        df["ctr_gain_potential"] = np.maximum(
            0, EDITORIAL_CRITERIA["target_ctr_gain"] - (df["ctr"] - median_ctr)
        )
        df["below_median_ctr"] = (df["ctr"] < median_ctr).astype(int)
    else:
...
df_train = create_editorial_features(df_train)
df_val = create_editorial_features(df_val)
df_test = create_editorial_features(df_test)
...
# Additional target for headline rewriting
median_ctr = df_train["ctr"].median()
df_train["needs_rewrite"] = (df_train["ctr"] < median_ctr).astype(int)
df_val["needs_rewrite"] = (df_val["ctr"] < median_ctr).astype(int)
...
processing_metadata = {
    ...
    "target_statistics": {
        ...
        "median_ctr": float(df_train["ctr"].median()),
