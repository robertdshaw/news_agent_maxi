...
# ============================================================================
# STEP 2: LOAD MODEL FOR CTR PREDICTION
# ============================================================================
print("\nStep 2: Loading trained model for CTR prediction...")

try:
    with open(MODEL_DIR / "xgboost_optimized_model.pkl", "rb") as f:
        trained_model = pickle.load(f)

    # Load feature data for CTR prediction
...
# Function to predict CTR using the trained model
def predict_ctr_with_model(article_metadata, features_df, model):
    """Predict CTR using the trained model for articles"""
    if model is None or features_df is None:
        return article_metadata.get("ctr", 0.035)  # Use actual CTR or default

    try:
        # Find matching features for this article
        matching_features = features_df[features_df.index.isin([article_metadata.name])]
        if len(matching_features) == 0:
            return article_metadata.get("ctr", 0.035)

        # Predict engagement probability
        prob = model.predict_proba(matching_features)[:, 1][0]

        # Convert engagement probability to estimated CTR
        # Based on the relationship: high_engagement_threshold = 0.15
        # Articles with >0.5 engagement probability typically have CTR > 0.05
        estimated_ctr = max(
            0.01, prob * 0.1
        )  # Scale probability to reasonable CTR range

        return estimated_ctr

    except Exception as e:
        print(
            f"Warning: CTR prediction failed for article {article_metadata.get('newsID', 'unknown')}: {e}"
        )
        return article_metadata.get("ctr", 0.035)


def create_features_for_headline(title, abstract="", category="news"):
    """Create feature vector for a headline to use with the trained model"""
    features = {}
    ...
    # Editorial scores (simplified versions)
    features["editorial_readability_score"] = (
        features["title_reading_ease"] / 100
    ) * 0.3
    ...
    try:
        ...
        print(f"Warning: Could not encode category {category}: {e}")
...
def predict_ctr_for_headline(title, abstract="", category="news", model=None):
    """Predict CTR for a specific headline using the trained model"""
    if model is None:
        return 0.035  # Default CTR

    try:
        # Create features for this headline
        features = create_features_for_headline(title, abstract, category)
        ...
        if hasattr(model, "feature_names_in_"):
            ...
            # Use all features in sorted order as fallback
            feature_vector = [features[k] for k in sorted(features.keys())]

        # Make prediction
        feature_array = np.array(feature_vector).reshape(1, -1)
        engagement_prob = model.predict_proba(feature_array)[:, 1][0]
...
for idx, article in low_performing_headlines.iterrows():
    # Predict CTR for original article using model
    if trained_model is not None and X_train is not None:
        original_predicted_ctr = predict_ctr_with_model(article, X_train, trained_model)
    else:
    ...
    for strategy, rewritten_title in variants.items():
        if rewritten_title != article["title"]:  # Only include actual rewrites
            ...
            # **KEY CHANGE: Use model to predict CTR for rewritten headline**
            if trained_model is not None:
                rewritten_predicted_ctr = predict_ctr_for_headline(
                    rewritten_title,
