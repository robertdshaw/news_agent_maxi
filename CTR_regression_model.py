...
try:
    ...
    # For train/val: All loaded CTRs are considered 'real' for training/validation purposes,
    # as they are derived from historical data (including true 0s or fills for no impressions).
    train_flags = pd.Series(True, index=y_train.index)
    val_flags = pd.Series(True, index=y_val.index)

    # For test: CTR is a placeholder, so not 'real' for evaluation against actuals or for training.
    test_flags = pd.Series(False, index=y_test.index)
...
test_rmse = np.sqrt(np.mean((preds - y_val_filtered) ** 2))
test_mae = np.mean(np.abs(preds - y_val_filtered))
val_r2 = r2_score(y_val_filtered, preds)
